...
eq = Equilibrium.load(
    "/scratch/gpfs/rg6256/DESC_ball_tests/OP_ball3_H100/eq00_finite_beta_and_iota.h5"
)
# eq = Equilibrium.load("/scratch/gpfs/rg6256/DESC_ball_tests/OP_ball3_H100/eq_final_new_033.h5")
...
for i in range(len(alpha_arr)):
    ...
    for j in range(len(surfaces)):
        ...
        ball_data0 = eq.compute(["ideal ballooning lambda"], grid=sfl_grid)[
...
np.savez(
    "./OP_ball3_H100/gamma_max_OP_ball3_H100_hres_correct_norm_initial_new_033_wider_zeta1_wider_zeta0",
    alpha=alpha_arr,
    rho=np.array(surfaces),
    lambda_max=gamma_max,
)
# np.savez("./OP_ball3_H100/gamma_max_OP_ball3_H100_hres_correct_norm_optimized_new_033_wider_zeta1_wider_zeta0", alpha = alpha_arr, rho =np.array(surfaces), lambda_max = gamma_max)
